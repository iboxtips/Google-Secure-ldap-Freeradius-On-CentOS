...
sed -i "s|ACCESS_ALLOWED_CIDR|$ACCESS_ALLOWED_CIDR|g" /etc/raddb/clients.conf
sed -i "s|SHARED_SECRET|$SHARED_SECRET|g" /etc/raddb/clients.conf

sed -i "s|BASE_DOMAIN|$BASE_DOMAIN|g" /etc/raddb/proxy.conf
sed -i "s|DOMAIN_EXTENSION|$DOMAIN_EXTENSION|g" /etc/raddb/proxy.conf

sed -i "s|GOOGLE_LDAP_PASSWORD|$GOOGLE_LDAP_PASSWORD|g" /etc/raddb/mods-available/ldap
sed -i "s|GOOGLE_LDAP_USERNAME|$GOOGLE_LDAP_USERNAME|g" /etc/raddb/mods-available/ldap

# add support to second level like: .com.br, .com.ar
sed -i "s|BASE_DOMAIN|$BASE_DOMAIN|g" /etc/raddb/mods-available/ldap
if [[ ${DOMAIN_EXTENSION} =~ [.] ]]; then
    DOMAIN_EXTENSION=$( echo $DOMAIN_EXTENSION | awk -F'.' '{print $1",dc="$2}' )
fi
sed -i "s|DOMAIN_EXTENSION|$DOMAIN_EXTENSION|g" /etc/raddb/mods-available/ldap

# Handle the certs
cp /certs/ldap-client.key /etc/raddb/certs/ldap-client.key
cp /certs/ldap-client.crt /etc/raddb/certs/ldap-client.crt
chown freerad:freerad /etc/raddb/certs/ldap-client*
